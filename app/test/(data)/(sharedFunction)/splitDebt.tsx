...
function splitExpense(
  ...
  const getMaxDebtorCreditor = (debts: { [userId: string]: number }) => {
    ...
    const creditor = creditors.reduce(
      (maxUserId, userId) =>
        debts[userId] > debts[maxUserId] ? userId : maxUserId,
      creditors[0],
    );
    const debtor = debtors.reduce(
      (minUserId, userId) =>
        debts[userId] < debts[minUserId] ? userId : minUserId,
      debtors[0],
    );
  ...
  while (Object.values(totalDebts).some((amount) => Math.abs(amount) > 0.01)) {
    ...
    splitDebt[creditor][debtor] =
      (splitDebt[creditor][debtor] || 0) + debtAmount;
    splitDebt[debtor][creditor] =
      (splitDebt[debtor][creditor] || 0) - debtAmount;
