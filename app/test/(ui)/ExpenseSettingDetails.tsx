    ...
    return (
        <div className="my-6 flex w-full flex-col items-center">
            ...
            {users.map((user: any) => {
                return (
                    <div
                        className="px-7 my-3 flex w-full items-center justify-between"
                        key={user.id}
                    >
                        ...
                        <input
                            className="relative h-5 w-5 rounded-full border-[1.5px] border-black hover:checked:border-black checked:border-black checked:focus:border-black checked:bg-highlight-60 focus:ring-highlight-60 checked:text-highlight-60 active:bg-highlight-60 checked:before:block checked:before:absolute checked:before:bg-highlight-60 checked:before:w-3 checked:before:h-3 checked:before:top-[50%] checked:before:left-[50%] checked:before:translate-y-[-50%] checked:before:translate-x-[-50%] checked:before:rounded-full"
                            type="radio"
                            id={user.name}
                            name="payer"
                            value={user.name}
...
export function ExpenseSettingStepThree({
    ...
    return (
        <div className="my-6 flex w-full flex-col items-center">
            <div className="px-3 mb-5 flex justify-between items-center w-full">
                <div className="w-12" />
            ...
            {users.map((user: any) => {
                ...
                const amountValue = isChecked ? expenseData.sharers.filter((sharer: any) => sharer.id === user.id)[0].amount : '0'
                return (
                    <div
                        className="px-7 my-3 flex w-full items-center justify-between"
                        key={user.id}
                    >
                        <div className="flex items-center gap-4">
                            ...
                            />
                            <div>{user.name}</div>
                            <input className="w-48 border-0 border-b border-grey-500 bg-transparent pb-1 pl-0 focus:border-b focus:border-highlight-40 focus:ring-0" type="text" onChange={() => { }} value={amountValue} />
                        </div>
                        <input
                            className="relative h-5 w-5 rounded-full border-[1.5px] border-black hover:checked:border-black checked:border-black checked:focus:border-black checked:bg-highlight-60 focus:ring-highlight-60 checked:text-highlight-60 active:bg-highlight-60 checked:before:block checked:before:absolute checked:before:bg-highlight-60 checked:before:w-3 checked:before:h-3 checked:before:top-[50%] checked:before:left-[50%] checked:before:translate-y-[-50%] checked:before:translate-x-[-50%] checked:before:rounded-full"
                            type="radio"
