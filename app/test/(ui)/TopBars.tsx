...
export function TopGroupBar({ isBalancePage, groupData }: TopGroupBarProps) {
  ...
  return (
    <div className="fixed left-[50%] top-0 z-10 flex w-full -translate-x-[50%] items-center justify-center bg-highlight-50">
      <div className="flex w-full min-w-[320px] max-w-[800px] items-center justify-between  px-5 py-4 text-white">
        <div className="flex h-6 w-6 items-center justify-center">
          {isBalancePage && hasGroupData ? (
            <LoadingButton
              url={`/test/split/group/${groupData.id}`}
              className="flex"
            >
              <BackArrowIcon />
            </LoadingButton>
    ...
    <div className="fixed z-20 flex w-full items-center justify-center bg-highlight-50">
      <div className="flex w-full min-w-[320px] max-w-[800px] items-center justify-between px-5 py-4 text-white">
        ...
        <div className="h-6 w-6">
          {expenseData ? (
            <LoadingButton
              url={`/test/split/group/${id}/expense/${expenseData.id}/edit`}
              className="h-6 w-6"
            >
              <EditTwoIcon />
            </LoadingButton>
