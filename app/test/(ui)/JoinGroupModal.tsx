  ...
  const handleSave = async () => {
    ...
    const updatedUsers = tempUsers.map((user: GroupUser) => {
      return {
        ...
        picture:
          user.id === selectedUserId ? loginUserData.picture : user.picture,
        adoptable: user.adoptable,
      };
    });
    const updatedCreatorId =
      selectedUserId === groupData?.creatorId
        ? loginUserData.id
        : groupData?.creatorId;
    ...
    let updatedGroupExpenses = groupData.expenses
      ? groupData.expenses.map((expense) => ({
          ...expense,
          payerId:
            expense.payerId === selectedUserId
              ? loginUserData.id
              : expense.payerId,
          sharers: expense.sharers.map((sharer) =>
            sharer.id === selectedUserId
              ? { id: loginUserData.id, amount: sharer.amount }
              : sharer,
          ),
        }))
      : [];
    ...
    let updateExpenses = AllExpenses.map((expense: ExtendedExpense) => ({
      ...expense,
      payerId:
        expense.payerId === selectedUserId ? loginUserData.id : expense.payerId,
      sharers: expense.sharers.map((sharer) =>
        sharer.id === selectedUserId
          ? { id: loginUserData.id, amount: sharer.amount }
          : sharer,
      ...
      >
        ...
        >
          <div className="flex items-center justify-center gap-2 rounded-t-lg bg-highlight-60 py-2">
            ...
            <div className="text-normal max-w-[120px] truncate">
              {groupData?.name}
            </div>
          </div>
              ...
              >
                ...
                <label
                  ...
                  className={clsx('max-w-[130px] truncate', {
                    'text-neutrals-50':
                      !user.adoptable && user.id !== selectedUserId,
                  })}
                >
                  {user.name}
                </label>
